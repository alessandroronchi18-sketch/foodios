...
-- moltiplicato per €/kg medio dei formati = ricavo atteso. Diff = porzioni
-- sbagliate, omaggi non registrati, errori scontrino.
--
...
--   Determina la visibilita della voce menu "Inventario gusti".
-- metodo_produzione: come la sede registra la produzione.
--   stampi      = metodo attuale (riga ricetta + n stampi, scontrino match)
--   inventario  = metodo differenziale (PROD + RIMAN giornaliero per gusto)
...
-- quando l utente rinomina (riusiamo lo stesso pattern di stock_prodotti_finiti).
...
create table if not exists public.inventario_produzione (
  ...
  data            date not null,
  produzione_g    integer not null default 0 check (produzione_g >= 0),
  rimanenza_g    integer not null default 0 check (rimanenza_g >= 0),
  scarto_g        integer not null default 0 check (scarto_g >= 0),
  note            text,
  created_by      uuid references auth.users(id) on delete set null,
  updated_by      uuid references auth.users(id) on delete set null,
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now(),
  unique (organization_id, sede_id, gusto_nome, data)
);
...
-- ── 4. RLS ────────────────────────────────────────────────────────────────
-- Pattern coerente con stock_prodotti_finiti / trasferimenti: tutti i membri
-- dell organizzazione possono leggere, solo quelli con accesso write possono
-- modificare. Niente accesso cross-org.
...
-- ── 5. Audit log: aggancio al pattern user_data_audit esistente ───────────
-- Non instrumentiamo qui: l audit_log esistente copre user_data; per le tabelle
-- relazionali come questa, l istoria e tracciabile via updated_by + updated_at.
-- Se serve, in futuro un trigger AFTER INSERT/UPDATE puo scrivere su audit_log.

comment on table public.inventario_produzione is
  'Metodo produzione inventario differenziale: PROD/RIMAN giornaliero per gusto. Vedi sedi.metodo_produzione.';
