      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:isMobile?16:20,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200,position:"relative"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca ricetta…"
            ...
            onFocus={e=>{e.target.style.borderColor=T.brand;e.target.style.boxShadow="0 0 0 3px rgba(110,14,26,0.10)";}}
                        ...
                        {ing.ricette.map((r,ri)=>(
                          <div key={r} style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                            <div style={{width:8,height:8,borderRadius:"50%",flexShrink:0,
                              background:["#6E0E1A","#E07040","#B45309","#5B8FCE","#7B7B7B"][ri%5]}}/>
            ...
            {hasQta && (
              !confermando ? (
                <button onClick={()=>setConfermando(true)}
                  style={{padding:"14px",background:C.red,color:C.white,border:"none",borderRadius:10,fontWeight:800,fontSize:13,cursor:"pointer",boxShadow:"0 2px 8px rgba(110,14,26,0.3)"}}>
                  ✅ Conferma produzione
                </button>
              ) : (
          ...
          <div style={{display:"grid",gridTemplateColumns:"160px 1fr",gap:20,alignItems:"flex-start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {!venduto && !loading && !error && (
                <button onClick={handleAnalizza} style={{padding:"13px",background:C.red,color:C.white,border:"none",borderRadius:9,fontWeight:800,fontSize:13,cursor:"pointer",boxShadow:"0 2px 10px rgba(110,14,26,0.25)"}}>
    ...
    return (
      <div onClick={onClick}
        style={{background:T.bgCard, border:`1px solid ${isAlert?'rgba(110,14,26,0.28)':T.border}`,
          ...
          boxShadow:isAlert?"0 4px 14px rgba(110,14,26,0.14), 0 1px 3px rgba(15,23,42,0.05)":"0 1px 2px rgba(15,23,42,0.05), 0 4px 14px rgba(15,23,42,0.05)",
          cursor:"pointer", position:"relative", overflow:"hidden",
          transition:`box-shadow ${M.durBase} ${M.ease}, border-color ${M.durBase} ${M.ease}, transform ${M.durFast} ${M.ease}`}}
        onMouseEnter={e=>{e.currentTarget.style.boxShadow=isAlert?"0 12px 28px rgba(110,14,26,0.22)":"0 12px 28px rgba(15,23,42,0.10)";e.currentTarget.style.borderColor=isAlert?'rgba(110,14,26,0.52)':T.borderStr;e.currentTarget.style.transform="translateY(-2px)";}}
        onMouseLeave={e=>{e.currentTarget.style.boxShadow=isAlert?"0 4px 14px rgba(110,14,26,0.14), 0 1px 3px rgba(15,23,42,0.05)":"0 1px 2px rgba(15,23,42,0.05), 0 4px 14px rgba(15,23,42,0.05)";e.currentTarget.style.borderColor=isAlert?'rgba(110,14,26,0.28)':T.border;e.currentTarget.style.transform="translateY(0)";}}>
