...
/**
 * Classifica tutte le righe, e prepara **una frase sola** da dire a schermo
 * su quelle che non sono merce.
 *
 * Mai in silenzio: una riga tolta senza dirlo è una riga che nessuno andrà
 * mai a cercare.
 */
export function classificaRighe(righe = []) {
  const fuori = { [TIPO.CAMPIONE]: [], [TIPO.SERVIZIO]: [], [TIPO.RUMORE]: [], [TIPO.RESO]: [], [TIPO.OMAGGIO]: [] }
  const classificate = (Array.isArray(righe) ? righe : []).map((r, i) => {
    const c = classificaRiga(r)
    if (c.tipo !== TIPO.MERCE) fuori[c.tipo].push({ indice: i, nome: String(r?.nome || r?.descrizione || '').trim() })
    return { ...r, _classe: c }
  })
  return { righe: classificate, fuori, avvisi: avvisiDaFuori(fuori) }
}

const elenco = (v) => v.map(x => x.nome).filter(Boolean).join(', ')

function avvisiDaFuori(fuori) {
  const out = []
  const c = fuori[TIPO.CAMPIONE]
  if (c.length) {
    out.push(`${c.length === 1 ? 'Una riga è un campione e l’ho saltata' : `${c.length} righe sono campioni e le ho saltate`}${elenco(c) ? ` (${elenco(c)})` : ''}: hanno un prezzo simbolico che, entrando nel listino, farebbe crollare il food cost.`)
  }
  const s = fuori[TIPO.SERVIZIO]
  if (s.length) {
    out.push(`${s.length === 1 ? 'Una riga è un servizio' : `${s.length} righe sono servizi`}${elenco(s) ? ` (${elenco(s)})` : ''}: non entra in magazzino, ma resta un costo nel totale del documento.`)
  }
  const r = fuori[TIPO.RESO]
  if (r.length) {
    out.push(`${r.length === 1 ? 'Una riga è un reso' : `${r.length} righe sono resi`}${elenco(r) ? ` (${elenco(r)})` : ''}: scarico la merce e non tocco il prezzo di listino.`)
  }
  const o = fuori[TIPO.OMAGGIO]
  if (o.length) {
    out.push(`${o.length === 1 ? 'Una riga è in omaggio' : `${o.length} righe sono in omaggio`}${elenco(o) ? ` (${elenco(o)})` : ''}: la merce entra in magazzino e il prezzo resta quello vero, non zero.`)
  }
  const n = fuori[TIPO.RUMORE]
  if (n.length) {
    out.push(`${n.length === 1 ? 'Una riga era testo del documento' : `${n.length} righe erano testo del documento`} (avvisi, condizioni, coordinate bancarie) e non le ho considerate.`)
  }
  return out
}
