...
export async function exportRicettaPDF(ricetta, foodCost, ingCosti, nomeAttivita, emailUtente) {
  ...
  autoTable(doc, {
    ...
    theme: 'striped',
    headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 12, fontStyle: 'bold' },
...
export async function exportPLCompleto(dati, nomeAttivita, emailUtente) {
  ...
  if (insights.length) {
    ...
    autoTable(doc, {
      ...
      theme: 'plain',
      bodyStyles: { fontSize: 12, cellPadding: 3 },
      columnStyles: {
...
export async function exportSimulatorePrezzi(dati, nomeAttivita, emailUtente) {
  ...
  if (rec.length) {
    ...
    autoTable(doc, {
      ...
      body: rec.map(r => ['•', r]),
      theme: 'plain',
      bodyStyles: { fontSize: 12, cellPadding: 2.5 },
...
export async function exportProduzione(dati, data, nomeAttivita, emailUtente) {
  ...
  autoTable(doc, {
    ...
    theme: 'plain',
    styles: { fontSize: 12, fontStyle: 'bold', cellPadding: 4 },
